...
from .nodes import nodes_read_data, nodes_math
...
class GraphFlow:
    ...
    def execute_downstream(self):
        ...
        def run_graph(messageSignal):
            """"""
            try:
                execution_order = self.get_execution_order(start_node)
                for node in execution_order:
                    if hasattr(node, 'execute'):
                        node.execute()
                    if hasattr(node, 'print_value'):
                        messageSignal.emit(node.print_value())
    ...
    def execute_all_nodes(self):
        ...
        # 找到所有没有输入的节点作为起始点
        start_nodes = [node for node in all_nodes if not any( #没有输入的就是初始节点
            port.connected_ports()
            for port in node.inputs().values()
        )]

        if not start_nodes:
            self.messageSignal.emit("No executable start node is found in the diagram")
            return

        self.messageSignal.emit("=== Start executing the entire graph ===")

        def run_graph(start_node, messageSignal):
            """"""
            if start_node.disabled():
                return
            try:
        ...
        for start_node in start_nodes:
            thread = Thread(target=run_graph, args=(start_node, self.messageSignal))
            thread.start()
        # self.messageSignal.emit("=== 全部执行完成 ===\n")

    def get_execution_order(self, start_node):
        ...
        def visit_up(node):
            ...
            visited_up.add(node)
            execution_order_up.append(node)

        visited_down = set()
        execution_order_down = []
        def visit_down(node):
            ...
            for port in node.outputs().values():
                for connected_port in port.connected_ports():
                    visit_down(connected_port.node())

            visited_down.add(node)
            execution_order_down.append(node)
